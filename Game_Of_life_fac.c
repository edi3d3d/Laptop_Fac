...
void setup(FILE* data_file, int *size_l, int *size_c, int *nr_iteratii, bTree *tree_start, int *task, char ***board)
{
    ...
    for(int i=0;i<(*size_l); i++)
    {
        for(int j=0; j<(*size_c); j++)
        {
            ...
            for(int k=0; k<8; k++)
                if(i>=0 && i<size_l && j>=0 && j<size_l) nr_neighbours=((*board)[i + neighbours[k][0]][j + neighbours[k][1]] >> 4);
...
void node_processing (FILE* data_file, int size_l, int size_c, int nr_iteratii, bTree *tree_node, int task, char **board, int depth, char direction)
{
    int k=0;
    ...
    {
        for(int j=0; j<size_c; j++)
            printf("%c ", (board[i][j] & 0b00010000));
        printf("    ");

        for(int j=0; j<size_c; j++)
            printf("%c ", (board[i][j] & 0b00001111));
    ...
    for(int i=0; (*tree_node).cell[i].l!=-1; i++)
    {
        ...
        char board_value = board [cell_l] [cell_c];
        if((board_value & 0b00010000) == 1 ) state = -1;
        else state = 1;
        
        for(int j=0; j<=8; j++)
            if(cell_l>=0 && cell_l<size_l && cell_c>=0 && cell_c<size_c) board[ cell_l + neighbours[i][0] ][ cell_c + neighbours[i][1] ] += state;
    }

    task_cells(data_file, size_l, size_c, nr_iteratii, tree_node, task, board, depth+1);
